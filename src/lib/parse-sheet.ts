...
export function parseWorkbook(data: ArrayBuffer): ParsedSheet {
  ...
  markGrid.forEach((row, i) => {
    if (row.some((c) => /reg.*no/i.test(String(c ?? "")))) candidateIdxs.push(i);
  });
  if (!candidateIdxs.length) throw new Error("Could not find a REG. NO. / Reg.No header row");
  ...
  for (const idx of candidateIdxs) {
    ...
    header.forEach((cell, col) => {
      ...
      if ((h === "NAME" || h === "STUDENT'SNAME" || h === "STUDENTSNAME" || h === "NAMEOFTHESTUDENT") && !CODE_RE.test(h)) cName = col;
