...
const SHORT_NAMES: Record<string, string> = {
  ...
  CB3492: "Cryptography and Cyber Security",
  CCS335: "Cloud Computing",
  CCS366: "Software Testing Automation",
  NLP: "Natural Language Processing",
  DL: "Deep Learning for Vision",
...
const OFFICIAL_STAFF: Record<string, string> = {
  AL3501: "Mr.B.N.Sairam AP/AIML",
  AL3502: "Mrs.P.S.Indhumathi AP/AIML",
  CS3551: "Ms.S.Ilakkia AP/AIML",
  CB3492: "Dr.A.Vidhya HOD/AIML",
  CCS335: "Mr.B.N.Sairam AP/AIML",
  CCS366: "Mrs.P.S.Indhumathi AP/AIML",
  NLP: "Mr.B.N.Sairam AP/AIML",
  DL: "Mrs.P.S.Indhumathi AP/AIML",
  DC: "Ms.S.Ilakkia AP/AIML",
  CCS: "Dr.A.Vidhya HOD/AIML",
  CC: "Mr.B.N.Sairam AP/AIML",
  STA: "Mrs.P.S.Indhumathi AP/AIML",
};
...
function resolveSubjectStaff(code: string, info: Map<string, { name: string; staff: string }>): string {
  if (OFFICIAL_STAFF[code.toUpperCase()]) return OFFICIAL_STAFF[code.toUpperCase()];
  return info.get(code)?.staff ?? "";
}
...
function collectSubjectsFromRow(header: unknown[], skipCols: Set<number>, codeInfo: Map<string, { name: string; staff: string }>, preferBeforeCol?: number): { col: number; subject: Subject }[] {
  ...
  for (let col = 0; col < header.length; col++) {
    ...
    subjectCols.push({ col, subject: { code: extracted.code, name: resolveSubjectName(extracted.code, extracted.name, codeInfo), staff: resolveSubjectStaff(extracted.code, codeInfo) } });
  }
...
function collectSubjectsFromDateAndCodeRows(dateRow: unknown[], codeRow: unknown[], skipCols: Set<number>, codeInfo: Map<string, { name: string; staff: string }>): { col: number; subject: Subject }[] {
  ...
  for (let col = 0; col < maxCol; col++) {
    ...
    seenCodes.add(code);
    subjectCols.push({ col, subject: { code, name: resolveSubjectName(code, nameHint, codeInfo), staff: resolveSubjectStaff(code, codeInfo) } });
  }
  return subjectCols.sort((a, b) => a.col - b.col);
}
...
function extractHostelNames(grid: unknown[][]): { boys: string[]; girls: string[] } {
  ...
  for (const row of grid) {
    ...
    if (/boys\s*hostel/i.test(joined) || /hostel\s*boys/i.test(joined)) { mode = "boys"; continue; }
    if (/girls\s*hostel/i.test(joined) || /hostel\s*girls/i.test(joined)) { mode = "girls"; continue; }
    if (!mode) continue;
    if (/day\s*scholar|total\s*no|department|jeppiaar|pass\s*%|no\.?\s*of\s*students/i.test(joined)) { mode = null; continue; }
...
export function parseWorkbook(data: ArrayBuffer): ParsedSheet {
  ...
  markGrid.forEach((row, i) => {
    if (row.some((c) => {
      ...
      return /reg.*n[ou]/.test(s) || s.includes("regno") || s.includes("registernumber") || s.includes("registrationno") || s.includes("registrationnumber");
    ...
    for (const rIdx of scanRows) {
      ...
      for (let col = 0; col < row.length; col++) {
        ...
        if (m) {
          ...
          if (seen.has(code)) continue;
          seen.add(code);
          found.push({ col, subject: { code, name: resolveSubjectName(code, code, codeInfo), staff: resolveSubjectStaff(code, codeInfo) } });
          rowHadCode = true;
          continue;
        }
